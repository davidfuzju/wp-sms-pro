...
class taqnyat extends \WP_SMS\Gateway
{
    ...
    public function SendSMS()
    {
        ...
        $this->to = $recipients;
        $response = wp_remote_post($this->wsdl_link . 'v1/messages', ['headers' => ['Content-Type' => 'application/json', 'Authorization' => "Bearer {$this->has_key}"], 'timeout' => 30, 'body' => wp_json_encode(['recipients' => $this->to, 'sender' => $this->from, 'body' => $this->msg])]);
        // Check gateway credit
    ...
    public function GetCredit()
    {
        ...
        $response = wp_remote_get($this->wsdl_link . '/account/balance', ['headers' => ['Authorization' => "Bearer {$this->has_key}"]]);
        if (is_wp_error($response)) {
