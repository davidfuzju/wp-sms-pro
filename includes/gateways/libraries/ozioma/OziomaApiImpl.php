...
namespace {
    class OziomaApiImpl
    {
        ...
        {
            return array('username' => \urlencode($this->get_user_name()), 'password' => \urlencode($this->get_password()), 'sender' => \urlencode($this->get_sender()), 'message' => \urlencode($this->get_message()), 'recipient' => \urlencode($this->get_recipient()), 'remoteOp' => \urlencode($this->remote_op));
        }
        public function check_balance()
        {
            $this->remote_op = 'bal';
            $params = array('username' => \urlencode($this->get_user_name()), 'password' => \urlencode($this->get_password()), 'balance' => \urlencode(\true), 'remoteOp' => \urlencode($this->remote_op));
        ...
        private function parse_response($response)
        {
            ...
            $response_arr = \json_decode(\stripcslashes($response), \true);
            if ($response_arr !== NULL) {
                ...
                if ($this->remote_op == 'bal') {
                    ...
                    if ($this->remote_op == 'snd') {
                        //test for send operation
                        if ($this->status_code == "000040") {
                            //sent successful
                        ...
                        if ($this->remote_op == 'msg') {
                            //fetch message by id
                            if ($this->status_code != "000100") {
                                //is message id correct
                                $this->total_charged = \floatval($response_arr['charged']);
                                $this->total_failed = \floatval($response_arr['failed']);
                                $this->failed_nos = $response_arr['failedNos'];
                                $this->message_id = $response_arr['messageId'];
        ...
        {
            $this->remote_op = 'msg';
            $params = array('username' => \urlencode($this->get_user_name()), 'password' => \urlencode($this->get_password()), 'messageId' => \urlencode($id), 'remoteOp' => \urlencode($this->remote_op));
