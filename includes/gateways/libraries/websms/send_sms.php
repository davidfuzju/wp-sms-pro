...
namespace {
    ...
    try {
        ...
        // show success
        \print_r(array("Status          : " . $Response->getStatusCode(), "StatusMessage   : " . $Response->getStatusMessage(), "TransferId      : " . $Response->getTransferId(), "ClientMessageId : " . ($Response->getClientMessageId() ? $Response->getClientMessageId() : '<NOT SET>')));
        ...
        echo $e;
        // possibility to handle API status codes $e->getCode()
    ...
    function binary_sms_sample($recipientAddressList)
    {
        ...
        $messageContentSegments = array(\base64_encode(\pack("c*", 0x5, 0x0, 0x3, 0xfc, 0x2, 0x1, 0x5a, 0x75, 0x73, 0x61, 0x6d, 0x6d, 0x65, 0x6e)), \base64_encode(\pack("c*", 0x5, 0x0, 0x3, 0xfc, 0x2, 0x2, 0x67, 0x65, 0x66, 0xc3, 0xbc, 0x67, 0x74, 0x2e)));
        $userDataHeaderPresent = \true;
        # Binary Data includes UserDataHeader for e.G.: PDU sms (Concatenation)
