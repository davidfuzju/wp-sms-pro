 ...
 */
namespace WPSmsPro\Vendor\apimatic\jsonmapper;
        ...
        foreach ($json as $key => $jvalue) {
            ...
            if (!$hasProperty) {
                if ($this->bExceptionOnUndefinedProperty) {
                    throw new JsonMapperException('JSON property "' . $key . '" does not exist' . ' in object of type ' . $strClassName);
                }
                $isAdditional = \true;
            ...
            if ($accessor === null) {
                if ($this->bExceptionOnUndefinedProperty) {
                    throw new JsonMapperException('JSON property "' . $key . '" has no public setter method' . ' in object of type ' . $strClassName);
                }
                $isAdditional = \true;
            ...
            if ($type === '') {
                throw new JsonMapperException('Empty type at property "' . $strClassName . '::$' . $key . '"');
            }
            $array = null;
            $subtype = null;
            if (\substr($type, -2) == '[]') {
    ...
    protected function checkMissingData($providedProperties, \ReflectionClass $rc)
    {
        foreach ($rc->getProperties() as $property) {
            ...
            if (isset($annotations['required']) && !isset($providedProperties[$property->name])) {
                throw new JsonMapperException('Required property "' . $property->name . '" of class ' . $rc->getName() . ' is missing in JSON data');
            }
        }
    }
    /**
    ...
    protected function getAdditionalPropertiesMethod(\ReflectionClass $rc)
    {
        if ($this->bExceptionOnUndefinedProperty === \false && $this->sAdditionalPropertiesCollectionMethod !== null) {
            ...
            try {
                ...
                throw new \InvalidArgumentException($this->sAdditionalPropertiesCollectionMethod . " method is not available on the given class.");
            }
        ...
        foreach ($json as $key => $jvalue) {
            if ($class === null) {
                ...
                if ($this->isFlatType(\gettype($jvalue))) {
                    ...
                    $array[$key] = $this->map($jvalue, $this->createInstance($class));
                }
            }
        }
        return $array;
        ...
        if ($rc->hasMethod($setter)) {
            ...
            if ($rmeth->isPublic()) {
                ...
                if (\count($rparams) > 0) {
                    ...
                    if ($pclass !== null) {
                        return array(\true, $rmeth, '\\' . $pclass->getName());
                    }
                }
                $docblock = $rmeth->getDocComment();
                $annotations = $this->parseAnnotations($docblock);
                if (!isset($annotations['param'][0])) {
                    return array(\true, $rmeth, null);
                }
                list($type) = \explode(' ', \trim($annotations['param'][0]));
                return array(\true, $rmeth, $type);
            }
        }
        $rprop = null;
        ...
        if ($rprop !== null) {
            if ($rprop->isPublic()) {
                ...
                if (!isset($annotations['var'][0])) {
                    return array(\true, $rprop, null);
                }
                //support "@var type description"
                list($type) = \explode(' ', $annotations['var'][0]);
                return array(\true, $rprop, $type);
            } else {
                //no setter, private property
                return array(\true, null, null);
            }
        }
