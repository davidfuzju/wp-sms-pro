...
class FaxController extends BaseController
{
     ...
     */
    public function sendFax($fileUrl, $messages)
    {
        ...
        APIHelper::appendUrlWithQueryParameters($_queryBuilder, array('file_url' => $fileUrl));
     ...
     * @throws APIException Thrown if API call fails
     */
    public function getFaxReceipt($messageId)
    {
        //check that all required arguments are provided
        if (!isset($messageId)) {
            throw new \InvalidArgumentException("One or more required arguments were NULL.");
     ...
     * @throws APIException Thrown if API call fails
     */
    public function getFaxHistory($dateFrom, $dateTo, $q, $order)
    {
        //check that all required arguments are provided
        ...
        //process optional query parameters
        APIHelper::appendUrlWithQueryParameters($_queryBuilder, array('date_from' => $dateFrom, 'date_to' => $dateTo, 'q' => $q, 'order' => $order));
        //validate and preprocess url
        $_queryUrl = APIHelper::cleanUrl($_queryBuilder);
        //prepare headers
        $_headers = array('user-agent' => 'ClickSendSDK');
     ...
     * @throws APIException Thrown if API call fails
     */
    public function calculatePrice($fileUrl, $messages)
    {
        //check that all required arguments are provided
        if (!isset($fileUrl, $messages)) {
            throw new \InvalidArgumentException("One or more required arguments were NULL.");
