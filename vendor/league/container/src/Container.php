...
namespace WPSmsPro\Vendor\League\Container;

use WPSmsPro\Vendor\Interop\Container\ContainerInterface as InteropContainerInterface;
use WPSmsPro\Vendor\League\Container\Argument\RawArgumentInterface;
use WPSmsPro\Vendor\League\Container\Definition\DefinitionFactory;
use WPSmsPro\Vendor\League\Container\Definition\DefinitionFactoryInterface;
use WPSmsPro\Vendor\League\Container\Definition\DefinitionInterface;
use WPSmsPro\Vendor\League\Container\Exception\NotFoundException;
use WPSmsPro\Vendor\League\Container\Inflector\InflectorAggregate;
use WPSmsPro\Vendor\League\Container\Inflector\InflectorAggregateInterface;
use WPSmsPro\Vendor\League\Container\ServiceProvider\ServiceProviderAggregate;
use WPSmsPro\Vendor\League\Container\ServiceProvider\ServiceProviderAggregateInterface;
class Container implements ContainerInterface
{
     ...
     */
    public function __construct(ServiceProviderAggregateInterface $providers = null, InflectorAggregateInterface $inflectors = null, DefinitionFactoryInterface $definitionFactory = null)
    {
        // set required dependencies
        $this->providers = \is_null($providers) ? (new ServiceProviderAggregate())->setContainer($this) : $providers->setContainer($this);
        $this->inflectors = \is_null($inflectors) ? (new InflectorAggregate())->setContainer($this) : $inflectors->setContainer($this);
        $this->definitionFactory = \is_null($definitionFactory) ? (new DefinitionFactory())->setContainer($this) : $definitionFactory->setContainer($this);
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function extend($alias)
    {
        ...
        throw new NotFoundException(\sprintf('Unable to extend alias (%s) as it is not being managed as a definition', $alias));
    }
    /**
     * {@inheritdoc}
     */
