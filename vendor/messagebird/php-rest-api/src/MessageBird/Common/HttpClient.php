...
namespace WPSmsPro\Vendor\MessageBird\Common;
...
class HttpClient
{
    ...
    public function __construct($endpoint, $timeout = 10, $connectionTimeout = 2, $headers = array())
    {
        ...
        if (!\is_int($timeout) || $timeout < 1) {
            throw new \InvalidArgumentException(\sprintf('Timeout must be an int > 0, got "%s".', \is_object($timeout) ? \get_class($timeout) : \gettype($timeout) . ' ' . \var_export($timeout, \true)));
        }
        $this->timeout = $timeout;
        if (!\is_int($connectionTimeout) || $connectionTimeout < 0) {
            throw new \InvalidArgumentException(\sprintf('Connection timeout must be an int >= 0, got "%s".', \is_object($connectionTimeout) ? \get_class($connectionTimeout) : \gettype($connectionTimeout) . ' ' . \var_export($connectionTimeout, \true)));
        }
    ...
    public function performHttpRequest($method, $resourceName, $query = null, $body = null)
    {
        ...
        $headers = array('User-agent: ' . \implode(' ', $this->userAgent), 'Accept: application/json', 'Content-Type: application/json', 'Accept-Charset: utf-8', \sprintf('Authorization: AccessKey %s', $this->Authentication->accessKey));
