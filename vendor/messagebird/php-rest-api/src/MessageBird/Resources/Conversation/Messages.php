...
use WPSmsPro\Vendor\MessageBird\Common\ResponseError;
use WPSmsPro\Vendor\MessageBird\Exceptions\HttpException;
use WPSmsPro\Vendor\MessageBird\Exceptions\RequestException;
use WPSmsPro\Vendor\MessageBird\Exceptions\ServerException;
use WPSmsPro\Vendor\MessageBird\Objects\BaseList;
use WPSmsPro\Vendor\MessageBird\Objects\Conversation\Message;
...
class Messages
{
    ...
    public function create($conversationId, $object, $query = null)
    {
        ...
        list(, , $body) = $this->httpClient->performHttpRequest(HttpClient::REQUEST_POST, $this->getResourceNameWithId($conversationId), $query, $body);
    ...
    {
        list($status, , $body) = $this->httpClient->performHttpRequest(HttpClient::REQUEST_GET, $this->getResourceNameWithId($conversationId), $parameters);
    ...
    {
        list($status, , $body) = $this->httpClient->performHttpRequest(HttpClient::REQUEST_GET, \sprintf(self::MESSAGE_RESOURCE_NAME, $messageId), $parameters);
        if ($status !== self::HTTP_STATUS_OK) {
            return $this->processRequest($body);
        }
        $body = \json_decode($body);
