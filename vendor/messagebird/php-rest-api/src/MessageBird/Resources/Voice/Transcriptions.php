...
use WPSmsPro\Vendor\MessageBird\Exceptions;
...
class Transcriptions
{
    ...
    {
        list(, , $body) = $this->HttpClient->performHttpRequest(Common\HttpClient::REQUEST_POST, "calls/{$callId}/legs/{$legId}/recordings/{$recordingId}/transcriptions");
    ...
    {
        list($status, , $body) = $this->HttpClient->performHttpRequest(Common\HttpClient::REQUEST_GET, "calls/{$callId}/legs/{$legId}/recordings/{$recordingId}/transcriptions", $parameters);
        if ($status === 200) {
    ...
    {
        list(, , $body) = $this->HttpClient->performHttpRequest(Common\HttpClient::REQUEST_GET, "calls/{$callId}/legs/{$legId}/recordings/{$recordingId}/transcriptions/{$transcriptionId}");
        return $this->processRequest($body);
    }
    /**
     * @param string $callId string
    ...
    {
        list($status, , $body) = $this->HttpClient->performHttpRequest(Common\HttpClient::REQUEST_GET, "calls/{$callId}/legs/{$legId}/recordings/{$recordingId}/transcriptions/{$transcriptionId}.txt");
        if ($status !== 200) {
            return $this->processRequest($body);
        }
        return $body;
