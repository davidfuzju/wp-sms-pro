    ...
    if (!\class_exists('Puc_v4p13_Vcs_BitBucketApi', \false)) {
        class Puc_v4p13_Vcs_BitBucketApi extends \Puc_v4p13_Vcs_Api
        {
            ...
            public function getBranch($branchName)
            {
                ...
                return new \Puc_v4p13_Vcs_Reference(array('name' => $ref, 'updated' => $branch->target->date, 'downloadUrl' => $this->getDownloadUrl($branch->name)));
            }
            /**
             * Get a specific tag.
             *
            ...
            public function getTag($tagName)
            {
                ...
                return new \Puc_v4p13_Vcs_Reference(array('name' => $tag->name, 'version' => \ltrim($tag->name, 'v'), 'updated' => $tag->target->date, 'downloadUrl' => $this->getDownloadUrl($tag->name)));
            }
            /**
            ...
            public function setAuthentication($credentials)
            {
                ...
                if (!empty($credentials) && !empty($credentials['consumer_key'])) {
                    $this->oauth = new \Puc_v4p13_OAuthSignature($credentials['consumer_key'], $credentials['consumer_secret']);
                } else {
                    $this->oauth = null;
                }
            }
